...
interface ProductCardProps {
  slug: "owner-database" | "buyer-leads";
}

export function ProductCard({ slug }: ProductCardProps) {
  const product = slug === "owner-database" 
    ? PRODUCTS.ownerDatabase 
    : PRODUCTS.buyerLeads;

  const isPremium = slug === "buyer-leads";
    ...
    <Card className="product-card flex flex-col">
      <CardContent className="pt-6 flex-1">
        <div className="flex items-start justify-between mb-4">
          ...
          {isPremium && <Badge variant="primary">Premium</Badge>}
        </div>
